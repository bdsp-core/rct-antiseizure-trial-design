...
legend({'IIC Burden','Sim-fit','NaN'},'FontSize',14)
%title(sprintf('%s IIC burden and Simulator fit ', extractBefore(file.name,"_")))
title('IIC burden and Simulator fit','FontSize',14)
...
ylabel('IIC burden (min/h)','FontSize',14)
%legend('SZ CNN Data ','Curve fit','NaN Val','IIC Gen from Log curve')
legend({'IIC Burden','Drug free IIC Burden','NaN Val'},'FontSize',14)
% title(sprintf('%s IIC burden and Drug free IIC Burden ', extractBefore(file.name,"_")))
title('IIC burden and Drug free IIC Burden','FontSize',14)
...
% subplot(6,1,5); 
% plot(x,y1_noise_norm,'LineWidth',1)
% ylim([-10 70])
% xlabel('Time in Hrs')
% ylabel('seizure burden (min/h)')
% legend('Normlaized(Lognormal+MA-Drug Concentration)')   
% title(sprintf('%s Normalized Seizure Burden Lognormal Calculation-Drug Concentation with MA noise', extractBefore(file.name,"_")))

% subplot(6,1,6); 
% plot(x,y1_log_MA_norm,'LineWidth',1)
% ylim([-10 70])
% xlabel('Time in Hrs')
% ylabel('seizure burden (min/h)')
% legend('Normlaized(Lognormal+MA)')   
% title(sprintf('%s Normalized Seizure Burden Lognormal Calculation with MA noise', extractBefore(file.name,"_")))

   
 saveas(fig,sprintf('C:\\Users\\CDAC_01\\Dropbox (Partners HealthCare)\\AED_TRIAL_PROJECT\\BadChannelDetetction_demo\\BadChannelDetetction_demo\\SAH\\50 Clients Point of interest\\Testing simulation with out EPS\\Code_Figures_Paper\\%s IIC_Burden_Log_Spike_Drug_Con.jpeg', extractBefore(file.name,"_")))
