...
legend({'IIC Burden','Lognormal fit accounting drug','Lognormal fit without accounting drug','NaN'},'FontSize',14)
%title(sprintf('%s IIC burden and Simulator fit ', extractBefore(file.name,"_")))
title('Simulator Fit With and Without Accounting Drug','FontSize',14)
...
% ylim([-10 70])
% xlabel('Time in Hrs')
% ylabel('IIC burden (min/h)')
% %legend('SZ CNN Data ','Curve fit','NaN Val','IIC Gen from Log curve')
% legend('IIC Burden','Lognormal fit without accounting drug','NaN Val')
% % title(sprintf('%s IIC burden and Drug free IIC Burden ', extractBefore(file.name,"_")))
% title('IIC Burden And Simulator Fit Without Accounting Drug')
...
title('Drug Concentration','FontSize',14)
 ...
 saveas(fig,sprintf('C:\\Users\\CDAC_01\\Dropbox (Partners HealthCare)\\AED_TRIAL_PROJECT\\BadChannelDetetction_demo\\BadChannelDetetction_demo\\SAH\\50 Clients Point of interest\\Testing simulation with out EPS\\Code_Figures_Paper\\Combine_Sim_Fit_With_Without_Drug_Effect_In_One_Plot.jpeg'))
